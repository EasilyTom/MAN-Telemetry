...
static int command_settings(int tty_fd, CANUSB_SPEED speed, CANUSB_MODE mode, CANUSB_FRAME frame)
{
  ...
  cmd_frame_len = 0;
  cmd_frame[cmd_frame_len++] = 0xaa;  //This signifies somthing
  cmd_frame[cmd_frame_len++] = 0x55;  //This might be data 
  cmd_frame[cmd_frame_len++] = 0x12;  //This might be end of data
  ...
  cmd_frame[cmd_frame_len++] = 0;
  cmd_frame[cmd_frame_len++] = 0;
  cmd_frame[cmd_frame_len++] = generate_checksum(&cmd_frame[2], 17); //Last two are for checksum
...
int main(int argc, char *argv[])
{
  ...
  init_csv_file(); //Emptying the old CSV file and init with headings\
  Not sure if this is the correct place to init it\
